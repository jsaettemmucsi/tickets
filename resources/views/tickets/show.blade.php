<x-app-layout>
        		...
        		<div class="max-w-5xl mx-auto text-sm mt-20">
					...
					<div class="grid grid-cols-2">
						<div class="grid grid-cols-2">
							...
							<x-ticket-field label="Site" field="site" value="{{ $ticket->requested_by?->site->name }}" disabled /> 
						...
						<div class="grid grid-cols-2">
							...
							<x-ticket-field label="Owner Group" field="owner_group" value="{{ $ticket->owned_group?->name }}" disabled /> 
	...
	<script>
		...
		fillSelect('onhold_reason', holdreasons, '{{ $ticket->onhold_reason }}');
		...
		requester.addEventListener('change', function() {
			users.forEach(function(user) {
				if (user.id == requester.value) {
					sites.forEach(function (sitex) {
						if (sitex.id == user.site_id) {
							site.value = sitex.name;
						}
					});
				}
			});
